...
export default function LightsonDetailAnimated({ eventItem }: EventDetailProps) {
    const [selectedImage, setSelectedImage] = useState<string | null>(null);
    const shareUrl = typeof window !== "undefined" ? window.location.href : "";
    const [searchQuery, setSearchQuery] = useState("");
    const [selectedMonthYear, setSelectedMonthYear] = useState("");
    const currentIndex = sonalightsonData.findIndex(event => event.slug === eventItem.slug);
    const prevEvent = sonalightsonData[currentIndex - 1] || null;
    const nextEvent = sonalightsonData[currentIndex + 1] || null;
    ...
    const filteredEvents = sonalightsonData.filter((event) => {
        // Month-Year Filter
    ...
    useEffect(() => {
        document.title = eventItem
            ? ` Sona Lights On | ${eventItem.title} | Sona School of Business and Management`
    ...
    return (
        ...
        >
            <div className="pt-8">
                ...
                >
                    <span className="text-lg">←</span> Back to Sona Lights On
                </Link>
            </div>
                ...
                <aside className="lg:col-span-1 flex flex-col space-y-4">
                    ...
                    <div className="flex-1 space-y-4">
                        {filteredEvents.slice(0, 3).map((event) => (
                            <Link
                                key={event.slug}
                                href={`/sona-lights-on/${event.slug}`}
                        ...
                        {filteredEvents.length > 3 && (
                            <Link
                                href="/sona-lights-on"// 👈 redirect target
                ...
                <>
                    ...
                    {currentDayItem && (
                        <div className="mb-10">
                            ...
                            <motion.div
                                className="text-gray-700 text-base sm:text-lg md:text-xl leading-relaxed text-justify mb-6"
                    ...
                    >
                        <div className="relative max-w-full max-h-full flex items-center justify-center">
                            ...
                            <button
                                ...
                                onClick={(e) => {
                                    e.stopPropagation();
                                    if (!eventItem.gallery || eventItem.gallery.length === 0) return;
                                    const currentIndex = eventItem.gallery.indexOf(selectedImage);
                                    const prevIndex = (currentIndex - 1 + eventItem.gallery.length) % eventItem.gallery.length;
                                    setSelectedImage(eventItem.gallery[prevIndex]);
                                }}
                            >
                                ←
                            </button>
                            ...
                            <button
                                ...
                                onClick={(e) => {
                                    ...
                                    if (!eventItem.gallery || eventItem.gallery.length === 0) return;
                                    const currentIndex = eventItem.gallery.indexOf(selectedImage);
                                    const nextIndex = (currentIndex + 1) % eventItem.gallery.length;
                                    setSelectedImage(eventItem.gallery[nextIndex]);
                                }}
                            >
                                →
                            </button>
            ...
            {(prevEvent || nextEvent) && (
                ...
                >
                    ...
                    {prevEvent && (
                        <a
                            href={`/sona-lights-on/${prevEvent.slug}`}
                    ...
                    {nextEvent && (
                        <a
                            href={`/sona-lights-on/${nextEvent.slug}`}
